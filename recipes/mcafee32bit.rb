...
#
return if registry_key_exists?("HKEY_LOCAL_MACHINE\\SOFTWARE\\McAfee\\Agent", :i386)

# Download McAfee installer
#
remote_file "#{Chef::Config[:file_cache_path]}\\#{basename}" do
  source "#{dirname_url}/#{basename}"
   not_if { registry_key_exists?("HKEY_LOCAL_MACHINE\\SOFTWARE\\McAfee\\Agent", :i386) }
end
...
execute "Check Add Remove Programs" do
  ...
  action :run
  only_if { registry_key_exists?("HKEY_LOCAL_MACHINE\\SOFTWARE\\McAfee\\Agent", :i386) }
end 
